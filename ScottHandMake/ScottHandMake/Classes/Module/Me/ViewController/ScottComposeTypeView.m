...
#define kAnimTranslationY ([UIScreen mainScreen].bounds.size.height * 0.5)

@interface ScottComposeTypeView ()<UIScrollViewDelegate, POPAnimationDelegate>
/// 按钮数组
@property (nonatomic, strong) NSMutableArray *menuButtons;

@property (nonatomic, copy) void(^completionBlock)(ScottComposeType *type);

/// 选中的按钮
@property (nonatomic, strong) ScottComposeTypeButton *selectedButton;

@end

@implementation ScottComposeTypeView {
    /// 按钮滚动视图图
...
- (instancetype)initWithSelectComposeType:(ScottComposeTypeBlock)composeBlock {
    ...
    if (self) {
        ...
        // 记录完成回调
        self.completionBlock = composeBlock;
    }
    return self;
}
...
/// 点击撰写按钮
- (void)clickComposeButton:(ScottComposeTypeButton *)button {
...
- (void)pop_animationDidStop:(POPAnimation *)anim finished:(BOOL)finished {
    ...
    if (self.completionBlock != nil) {
        self.completionBlock(self.selectedButton.composeType);
    }
...
- (void)setupComposeButtons {
    ...
    // 2. 水平偏移
    CGFloat offsetX = [UIScreen mainScreen].bounds.size.width * 0.3;
    
    int index = 0;
    for (ScottComposeType *type in [ScottComposeType composeTypeList]) {
        ScottComposeTypeButton *button = [[ScottComposeTypeButton alloc] init];
...
- (void)setupEffectView {
    ...
    // 2. 广告图片
    UIImageView *slogan = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"compose_slogan"]];
    ...
    [_closeButton setImage:[UIImage imageNamed:@"tabbar_compose_background_icon_close"] forState:UIControlStateNormal];
    ...
    [_returnButton setImage:[UIImage imageNamed:@"tabbar_compose_background_icon_return"] forState:UIControlStateNormal];
